...
export default function slackin ({
  ...
  // static files
  app.use(`${path}assets`, express.static(assets))

  // invite endpoint
  app.post(`${path}invite`, json(), (req, res, next) => {
    let chanId
    if (channels) {
  ...
  app.get(`${path}iframe/dialog`, (req, res) => {
    let large = 'large' in req.query
    let { name } = slack.org
    let { active, total } = slack.users
    if (!name) return res.send(404)
  ...
  app.get(`${path}.well-known/acme-challenge/:id`, (req, res) => {
    res.send(process.env.LETSENCRYPT_CHALLENGE)
  })

  // badge js
  app.use(`${path}slackin.js`, express.static(assets + '/badge.js'))

  // badge rendering
  app.get(`${path}badge.svg`, (req, res) => {
    res.type('svg')
